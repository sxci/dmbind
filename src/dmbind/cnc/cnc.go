...
func checkLogined() (ok bool) {
	...
	if strings.Contains(string(body), "七牛云存储") { return true }
	return
}
...
func getLoginVerify() (imgBase64, lt string, err error) {
	_, body, err := cncFetcher.Get("http://portal.chinanetcenter.com/cas/login?request_locale=zh_CN")
	if err != nil { return }
